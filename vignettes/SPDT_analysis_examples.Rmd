...
See  https://github.com/PAskey/SPDT#readme to get instructions on how to install package and ensure you are connected to the database server at work or through the vpn if you are at home. ONce installed use following code make tha package available for use.
...
Let's apply these filters and categories to create a data set that might be useable in a glm type analysis. To do this we create a factor variable of the release categories, and we round the expanded NetXN to an integer, since some distributions for count data (binomial, negative binomial will require a whole number).
...
plotdata <-glmdata%>%filter(Int.Age==2)
...
p1 = ggplot(plotdata, aes(x = Waterbody_Name, y = NetXN, group = Lk_yr, fill = Lk_yr, shape = SAR_cat))+
...
p2 = ggplot(plotdata, aes(x = Waterbody_Name, y = NetX_wt, group = Lk_yr, fill = Lk_yr, shape = SAR_cat))+
  geom_point(position = position_dodge(0.5), aes(size = SAR_cat))+
  scale_size_manual(values = c(3,3.5,4,5))+
...
#Fit the model
model = glmer.nb(NetXN ~ (1|Lk_yr) + 0 + SAR_cat, data = plotdata, control=glmerControl(optimizer="nlminbwrap", optCtrl=list(maxfun=1e5)))
...
model= lmer(NetX_FL ~ (1|Lk_yr) + 0 + SAR_cat, data = plotdata)
...
#Now look at weights
model= lmer(NetX_wt ~ (1|Lk_yr) + 0 + SAR_cat, data = plotdata)
...
model= glmer(p_mat ~ (1|Lk_yr) + 0 + SAR_cat, family = binomial, weights = N, data = plotdata)
